...
def load_UEA_dataset(path, dataset):
    ...
    # train_size = int(train_weka.num_instances / 10)
    # test_size = int(test_weka.num_instances / 10)
    ...
    for j in range(nb_dims):
        ...
        mean = numpy.nanmean(numpy.concatenate([train[:, j], test[:, j]]))
        var = numpy.nanvar(numpy.concatenate([train[:, j], test[:, j]]))
        if var != 0:
...
if __name__ == '__main__':
    ...
    # log_file = os.path.join('log', args.dataset + '.log')
    ...
    print('Model training finished, use time(s): ', (endtime - starttime).seconds)
