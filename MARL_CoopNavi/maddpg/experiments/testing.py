...
def test(arglist):
    ...
    fuzzer, trainers = sample_seeds(1000)
    with U.single_threaded_session():
        ...
        while current_fuzz_time - start_fuzz_time < 3600 * 12 and len(fuzzer.corpus) > 0:
            # get action
            action_n = [agent.action(obs) for agent, obs in zip(trainers,obs_n)]
            # environment step
            new_obs_n, rew_n, done_n, info_n = env.step(action_n)
            ...
            # if (done or terminal) and len(state_seqs) < STEPS:
            #     print('continue_')
            if (len(state_seqs) == STEPS):
                data = np.array(state_seqs).T.reshape((1, -1, STEPS))
