...
from UnsupervisedScalableRepresentationLearningTimeSeries import utils
from UnsupervisedScalableRepresentationLearningTimeSeries.losses import triplet_loss
from UnsupervisedScalableRepresentationLearningTimeSeries.networks import causal_cnn, lstm
    ...
    def fit_encoder(self, X, y=None, save_memory=False, verbose=False):
        ...
        while i < self.nb_steps:
            for batch in train_generator:
                if verbose:
                    print('Epoch:', epochs + 1, '|', 'Step:', str(i + 1) + '/' + str(self.nb_steps))
                if self.cuda:
