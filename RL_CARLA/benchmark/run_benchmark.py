...
from UnsupervisedScalableRepresentationLearningTimeSeries.cuz_utils.predict import load_model, predict, preprocess, \
    CONFIDENCE, STEPS
...
def run_single(env, weather, start, target, agent_maker, seed, replay=False, em_guide=True):
    dataset = 'RL_CARLA'
    best_step = STEPS
    model_path = 'UnsupervisedScalableRepresentationLearningTimeSeries/models'
    encoder = load_model(os.path.join(model_path, 'rl_model'), dataset)
    clusterer = joblib.load(os.path.join(model_path, 'cluster_model', dataset + '_' + str(best_step) + '.pkl'))
    ...
    if replay == True:
        ...
        crashF = open('/home/dell/wangkr/MDPFuzz/RL_CARLA/results/crashStateSeqV2.txt', mode='a')
        noCrashF = open('/home/dell/wangkr/MDPFuzz/RL_CARLA/results/noCrashStateSeqV2.txt', mode='a')

        crashF_40 = open('/home/dell/wangkr/MDPFuzz/RL_CARLA/results/crashStateSeqV2_40.txt', mode='a')
        noCrashF_40 = open('/home/dell/wangkr/MDPFuzz/RL_CARLA/results/noCrashStateSeqV2_40.txt', mode='a')
        timeStamp = open('/home/dell/wangkr/MDPFuzz/RL_CARLA/results/timeStamp.txt', mode='a')
        ...
        while len(env.fuzzer.corpus) > 0:
            ...
            while env.tick():
                ...
                if(len(sequence) == STEPS):
                    data = np.array(sequence).T.reshape((1, -1, STEPS))
                    data = preprocess(data, data.shape[1])
                    label, prob = predict(encoder, clusterer, data)
